...
@interface CustiomControlView () <ZFSliderViewDelegate>
/// 竖屏控制层的View
@property (nonatomic, strong) ZFCustomerPortraitControlView *portraitControlView;
/// 横屏控制层的View
...
@property (nonatomic, strong) UIImageView *currentPlayedImgView;
...
@property (nonatomic, assign) NSTimeInterval liveCurrentTime;

@property (nonatomic, assign) NSTimeInterval liveCurrentChangeTime;


@end

@implementation CustiomControlView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.autoHiddenTimeInterval = 2000.5;
...
- (void)layoutSubviews {
    ...
    self.currentPlayedImgView.frame = self.bounds;
...
- (void)addAllSubViews {
    ...
    [self addSubview:self.currentPlayedImgView];
}
...
- (void)gestureBeganPan:(ZFPlayerGestureControl *)gestureControl panDirection:(ZFPanDirection)direction panLocation:(ZFPanLocation)location {
    if (direction == ZFPanDirectionH) {
        if (self.isLive) {
            self.sumTime = self.liveTotalTime - self.liveCurrentChangeTime;
        }else {
...
- (void)gestureChangedPan:(ZFPlayerGestureControl *)gestureControl panDirection:(ZFPanDirection)direction panLocation:(ZFPanLocation)location withVelocity:(CGPoint)velocity {
    if (direction == ZFPanDirectionH) {
        if (self.isLive) {
            ...
            self.liveCurrentChangeTime -= velocity.x / 200;
            // 需要限定sumTime的范围
            NSTimeInterval totalMovieDuration = self.liveTotalTime;
...
- (void)gestureEndedPan:(ZFPlayerGestureControl *)gestureControl panDirection:(ZFPanDirection)direction panLocation:(ZFPanLocation)location {
    @weakify(self)
    if (self.isLive) {
        if (direction == ZFPanDirectionH && self.sumTime >= 0 && self.liveTotalTime > 0) {
...
- (void)videoPlayer:(ZFPlayerController *)videoPlayer currentTime:(NSTimeInterval)currentTime totalTime:(NSTimeInterval)totalTime {
    ...
    NSLog(@"currentTimeString:%@---->currentTime:%d",currentTimeString,currentTime);
    if (totalTime == 0 && videoPlayer.progress < 0.1 && currentTime > 0) {
        
        self.liveTotalTime = currentTime + self.currentLivePlayedTime;
//        if (self.liveCurrentChangeTime == -1) {
//            self.liveCurrentChangeTime = self.liveTotalTime;
//        }
        self.portraitControlView.currentChangeTime = self.liveCurrentChangeTime;
        self.isLive = YES;
        if (currentTime > 1) {
            self.currentPlayedImgView.hidden = YES;
        }
...
- (void)videoPlayer:(ZFPlayerController *)videoPlayer orientationDidChanged:(ZFOrientationObserver *)observer {
    ...
    self.currentPlayedImgView.frame = self.player.currentPlayerManager.view.bounds;
}
...
- (void)sliderValueChangingValue:(CGFloat)value isForward:(BOOL)forward {
    ...
    if (self.isLive) {
        draggedTime = [ZFUtilities convertTimeSecond:self.liveTotalTime*value];
        totalTime = [ZFUtilities convertTimeSecond:self.liveTotalTime];
    }
...
- (void)setPlayer:(ZFPlayerController *)player {
    ...
    self.currentPlayedImgView.frame = player.currentPlayerManager.view.bounds;
    self.currentPlayedImgView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
- (UIImageView *)currentPlayedImgView {
    if (!_currentPlayedImgView) {
        _currentPlayedImgView = [[UIImageView alloc] init];
        _currentPlayedImgView.userInteractionEnabled = YES;
        _currentPlayedImgView.hidden = YES;
    }
    return _currentPlayedImgView;
}
...
- (ZFCustomerPortraitControlView *)portraitControlView {
    if (!_portraitControlView) {
        ...
        _portraitControlView.sliderValueChanged = ^(CGFloat value) {
            ...
            self.liveCurrentChangeTime = disTime;
        };
        
        _portraitControlView.sliderValueChangingLive = ^(CGFloat value, BOOL forward, NSString * _Nonnull sliderTimeString) {
            @strongify(self)
            [self cancelAutoFadeOutControlView];
            //设置当前的时间

        };
        _portraitControlView.sliderTapped = ^(CGFloat value) {
            @strongify(self)
            CGFloat disTime = self.liveTotalTime * (1-value);
            self.liveCurrentChangeTime = disTime;
        };
